...
- 2026-08-05（二）：`trade_gap_ms` 负值截断（clamp）到 0。真实数据核验（20250702
  ...
  语义一致，无需改动。另修正前条"午休表现为大 gap"的说法：午休期间不产生面板行，
  流动性好的标的午后开盘数秒内即恢复成交（落入首行快照的相位偏移窗），gap 被
  重置为 0，午休并不必然以大 gap 呈现（仅午后恢复安静时才会）。20250702 实测
  588000 分布：0 值 2739/4800（57%，刚发生）、(0,1s] 1603、(1,3s] 421、>3s 仅 37。
